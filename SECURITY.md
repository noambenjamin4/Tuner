...
2. **Production (proxy mode)**: the route handlers proxy to a standalone Node server (`server/server.js`). Two backends: a Render fallback (`DOWNLOADER_REMOTE_URL`/`DOWNLOADER_API_KEY`) and a **Home Bridge** on the operator's Mac reached over a Cloudflare tunnel (`DOWNLOADER_HOME_KEY` + a `bridgeUrl` read from Vercel Edge Config). `pickBackend()` prefers the home bridge (residential IP, not YouTube-bot-walled) and falls back to Render; the returned `jobId` is namespaced `home_`/`remote_` so status/file requests hit the same backend (`backendForJob`, strict UUID-validated). The Edge Config `bridgeUrl` is **host-pinned to `*.trycloudflare.com`** (`isTrustedBridgeUrl`) so a tampered value can't point the proxy at an arbitrary server (SSRF). The download server itself does the spawning.

The converter also does MP4 video (H.264 ≤1080p, `--max-filesize 2G`), YouTube playlists (`--flat-playlist`, ≤50), and Spotify (metadata read from Spotify's public embed HTML on Vercel — no auth — then each track downloaded via a yt-dlp `ytsearch1:<query>` search). Playlist/Spotify enumerate endpoints are separately per-IP rate-limited (20/10 min).
...
- **Per-IP rate limiting on the proxy.** `lib/server/rate-limit.ts` caps job starts per IP (80 per 10-minute sliding window — sized to clear a full 50-track batch, with downstream bridge caps as the real bound) and enumerate calls (20/10 min) on a separate bucket, with a periodic sweep so the IP map can't grow unbounded. Applied before the request is forwarded.
...
9. **Spotify search query → yt-dlp — NOT injectable.** For Spotify-matched tracks the client sends a `query` (e.g. `"Artist Title"`) instead of a URL. `startJobSchema` accepts exactly one of `url`/`query`, caps the query at 300 chars, and rejects control characters (`/^[^\x00-\x1f\x7f]+$/`). The download server prefixes it as a single argv element `ytsearch1:<query>` after `--` with `shell:false`, so — like the URL path — no shell/flag injection is possible even without the regex.
10. **Spotify enumerate SSRF — NOT exploitable.** `app/api/spotify/route.ts` fetches only `https://open.spotify.com/embed/{kind}/{id}`, where `{kind}` ∈ `{playlist,album,track}` and `{id}` matches a strict base62 pattern (`validateSpotifyUrl`) — no user-controlled host/path/scheme reaches the fetch. The parsed embed JSON is walked structurally (no `eval`, no prototype pollution) and the endpoint returns only `{title, artist}` pairs (≤50).
11. **Edge Config bridge URL — host-pinned.** `bridgeUrl` (written by the Mac bridge via the operator's Vercel CLI auth) is only honored if it's `https://` on `*.trycloudflare.com`; otherwise the proxy falls back. No secret is stored in the tracked repo — the bridge's `API_KEY`/`EDGE_CONFIG_ID` live in a gitignored `scripts/tunebad-bridge.env` (see `scripts/tunebad-bridge.env.example`), and `DOWNLOADER_HOME_KEY` is a server-only Vercel env.
12. **CSP.** Set statically in `next.config.mjs` (compatible with prerendered pages): `script-src 'self' 'unsafe-inline' 'wasm-unsafe-eval'` (`'unsafe-eval'` only in dev for React Refresh), `object-src 'none'`, `frame-ancestors 'none'`, `connect-src 'self' https://*.supabase.co wss://*.supabase.co`. The Render downloader origin does **not** need to appear here: the browser only ever talks to `/api/youtube*` on the same origin — the Vercel server function is the one making the outbound `fetch` to Render, which isn't subject to the page's CSP. Also sets `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer`, `X-Frame-Options: DENY`. `'unsafe-inline'` for scripts is required because the pages are statically prerendered (a per-request nonce can't match build-time HTML); the residual XSS risk is low since the app has no HTML-injection sinks.
